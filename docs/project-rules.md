...
## 0. 基本方針 (Fundamental Policy)
- **project-rules.mdへの準拠**: 開発は本ドキュメント（`docs/project-rules.md`）の内容に準拠して行う。
- **ルールの優先順位**: 他のドキュメントと矛盾が生じた場合、本ドキュメントのルールを優先する。
- **事前確認**: 開発着手前に本ドキュメントの内容を確認すること。
...
- **テーブル命名規則（個別アプリケーション）**:
  ...
  - **規則**: `{table_name}_{app_name}` または `AppPrefix{ModelName}`
  - **例**: Zaikoアプリのアイテムテーブルの場合 → `zaiko_items` (Model: `ZaikoItem`)

## 6. 認証・プロフィール設計 (Authentication & Profile)

- **統合認証 (SSO like)**:
  - プロジェクト全体で単一のSupabase Authインスタンスを使用し、ログイン状態を共有する。
  - ユーザーが一度ログインすれば、どの個別アプリ（`zaiko`, `other-app`）に遷移してもログイン状態が維持される。

- **共通プロフィール (`Profile` Table)**:
  - `public` スキーマに `Profile` (`profiles`) テーブルを作成し、`auth.users` と1:1で同期する。
  - **役割**: 全アプリ共通のユーザー情報（表示名、アバターURLなど）を管理する。
  - **アプリ利用状況**: ユーザーがどのアプリを利用開始しているかを管理するフィールド（例: `joinedApps` 配列など）を持たせる。

- **個別アプリデータとの連携**:
  - 個別アプリのデータ（例: `ZaikoFamilyMember`）は、共通の `Profile.id` (User ID) を外部キーとして参照する。

## 7. レイアウト・デザイン共通ルール
- **Simpro共通レイアウトの適用範囲**:
  - Simproプロジェクトとして用意しているヘッダーやフッターなどのレイアウト（ルートの `layout.tsx` 等）は、個別のアプリケーション（`/app/(app)/...` 以下）には適用しない。描画を行わないこと。
- **個別アプリのレイアウト**:
  - 個別のアプリケーションには、それぞれのアプリの用途・デザインに合わせた専用のレイアウトを用意する。
