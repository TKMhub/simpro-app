...
import { ZAIKO_CATEGORIES, getInventoryStatus } from '../_lib/zaiko-constants';
import { ZaikoItem } from '../_lib/types';
import { updateZaikoItem, deleteZaikoItem } from '../_lib/actions';

interface ZaikoDashboardClientProps {
  initialItems: ZaikoItem[];
}

export default function ZaikoDashboardClient({ initialItems }: ZaikoDashboardClientProps) {
  const router = useRouter();
  const [items, setItems] = useState<ZaikoItem[]>(initialItems);
  const [filter, setFilter] = useState('all');
  const [editingItem, setEditingItem] = useState<ZaikoItem | null>(null);

  // Server Componentからのデータ更新を反映
  useEffect(() => {
  ...
  // Calculate status for display and filtering
  ...
  const lowStockCount = items.filter(i => {
      const status = getInventoryStatus(i.quantity, i.threshold);
  ...
  const handleUpdateQuantity = async (delta: number) => {
    ...
    const newQuantity = Math.max(0, editingItem.quantity + delta);
    const newItem = { ...editingItem, quantity: newQuantity };
    ...
    // Server Update
    try {
        await updateZaikoItem(newItem.id, { quantity: newQuantity });
    } catch (e) {
